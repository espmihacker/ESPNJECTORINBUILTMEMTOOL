...
void DrawESP(ESP esp, int screenWidth, int screenHeight) {
     if(isESP) {
    esp.DrawText(Color(0, 255, 0, 255),
                 output, //Green This signature alwayes show in your esp view, But easyly can chnage by hex editor throwg
    ...
    if(response.Success) {
      ...
      /*  if (ActorRenderTime == LocalRenderTime)
        {
            if(playerstatus(status) == "Aiming")
                DrawString("Player Aiming at you", 40, (S_width / 2) - 200, (S_height / 9), 1.f, 0.f, 0.f, 1.f);
        ...
        if (botCount + playerCount > 0) { //This Name show red when exixst nearby enemy in your radar AntiRemove
            sprintf(extra, "Abenk", playerCount,botCount);
